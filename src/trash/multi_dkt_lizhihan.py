...
class MultiDkt():
	def __init__(self, num_skills, batch_size=124, hidden_units=100, optimizer='adam', dropout_rate=0.5):
		...
		def weighted_mean(y_true, y_pred):
			...
			# 加权平均
			score = tf.math.softmax(2 * (1- tf.math.sigmoid(y_pred)))
			y_pred = tf.math.sigmoid(tf.reduce_sum(score * y_pred, axis=-1, keepdims=True))
			
			return y_true, y_pred

		def WM_loss(y_true, y_pred):
			true, pred = weighted_mean(y_true, y_pred)
			return binary_crossentropy(true, pred)

		class WM_Auc(tf.keras.metrics.AUC):
			def update_state(self, y_true, y_pred, sample_weight=None):
				true, pred = weighted_mean(y_true, y_pred)
				super(WM_Auc, self).update_state(y_true=true, y_pred=pred, sample_weight=sample_weight)

		class WM_BinaryAccuracy(tf.keras.metrics.BinaryAccuracy):
			def update_state(self, y_true, y_pred, sample_weight=None):
				true, pred = weighted_mean(y_true, y_pred)
				super(WM_BinaryAccuracy, self).update_state(y_true=true, y_pred=pred, sample_weight=sample_weight)
		...
		#y = TimeDistributed(Dense(num_skills))(dropout)
		...
		#self.__model.compile(loss=WM_loss, optimizer=optimizer, metrics=[WM_Auc(name='auc'), WM_BinaryAccuracy(name='acc')])
		#self.__model.compile(run_eagerly=True, loss=WM_loss, optimizer=optimizer, metrics=[WM_Auc(name='auc'), WM_BinaryAccuracy(name='acc')])
